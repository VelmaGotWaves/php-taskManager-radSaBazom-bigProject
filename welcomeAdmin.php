...
#na load odradi funkcije uzmiTaskove, itd
#function uzmiTaskove($link){
    $sql = "SELECT * FROM task join korisnik on task.id=korisnik.id order by TaskID desc;";
      ...
      $i=0;
        ...
        while($row = mysqli_fetch_assoc($result)){
          $taskRezultat[$i] = array(
            ...
            "KorisnikCompleted"=>$row["KorisnikCompleted"],
            "Kategorija"=>$row["Kategorija"],
            "ImePosaljioca"=>$row["Ime"],
            "EmailPosaljioca"=>$row["Email"],
  ...
  #}
    ...
    $sql1 = "SELECT sum(a) as nezavrseni, sum(b)as zavrseni, sum(a+b) as ukupno FROM(select case when completed=0 then 1 else 0 end as a, case when completed =1 then 1 else 0 end as b from task)t;";
    $result1 = mysqli_query($link, $sql1);
    $resultNumRows1= mysqli_num_rows($result1);
    
    $taskCompletedCount = array();
    ////////////^big^boy^//////////////////////////////////////////////////////////////////////////////////
    if($resultNumRows1>0){
        while($row = mysqli_fetch_assoc($result1)){
          $taskCompletedCount = array(
            "Completed"=>$row["zavrseni"],
            "NotCompleted"=>$row["nezavrseni"],
            "All"=>$row["ukupno"],
          );
        }
    }
...
<div class="container">
 <div class="user-profile-area">
  ...
  <div class="side-wrapper">
   ...
   <div class="progress-status"><?php echo $taskCompletedCount['Completed'].'/'.$taskCompletedCount['All'];?></div>
   ...
   <div class="task-status">
    <div class="task-stat">
     <div class="task-number"><?php echo $taskCompletedCount['Completed'];?></div>
     <div class="task-condition">Completed</div>
     <div class="task-tasks">tasks</div>
    </div>
    <div class="task-stat">
     <div class="task-number"><?php echo $taskCompletedCount['NotCompleted'];?></div>
     <div class="task-condition">To do</div>
     <div class="task-tasks">tasks</div>
    </div>
    <div class="task-stat">
     <div class="task-number"><?php echo $taskCompletedCount['All'];?></div>
 ...
 <div class="main-area">
  ...
  <div class="main-container">
     ...
     <div class="msg msg-department anim-y">
      ...
      if($resultNumRows>0){
        for ($k=0; $k < $resultNumRows; $k++) { 
          // bice ovde jedan if marketing if desing itd
   ...
   <div class="mail-detail">
    <div class="mail-detail-header">
     <div class="mail-detail-profile">
      ...
      <div class="mail-detail-name"><?php echo $taskRezultat[0]["ImePosaljioca"]?></div>
            ...
            <form action="obrisiTask.php" method="post">
                <!-- ne salje nIdObrisanog uopste-->
                <input type="text" name="nId" id="nIdObrisanog" style="display:none">
                
              <button type="submit"class="mail-icons">
    ...
    <input type="text" name="nId" id="nIdObrisanog1" style="display:none">
    ...
    <div class="mail-textarea">
                ...
                <input type="text" name="nComment"placeholder="Write a comment...">
                <button type="submit" class="textarea-icons">
                  <div class="send">
  ...
  <script>
    ...
    for (let index = 0; index < konacniBrojRedova; index++) {
      ...
      sidePaneli[index]='<div class="mail-contents-subject"><input type="checkbox" data-checkbox-vrednost="'+konacniNiz[index]['TaskID']+'"name="msg1" id="mail'+konacniNiz[index]['TaskID']*1000+'" class="mail-choice"'+cek+' ><label for="mail'+konacniNiz[index]['TaskID']*1000+'"></label><div class="mail-contents-title">'+konacniNiz[index]["Naslov"]+'</div></div><div class="mail"><div class="mail-time"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-clock"><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></svg>'+konacniNiz[index]["Datum"]+'</div><div class="mail-inside">'+konacniNiz[index]["Telo"]+''+komentar+'</div><div class="mail-checklist"><input type="checkbox" name="msg2" id="mail'+((konacniNiz[index]['TaskID']*1000)+1)+'" class="mail-choice" '+cek1+'><label for="mail'+((konacniNiz[index]['TaskID']*1000)+1)+'">User completed this task.</label></div></div>'
    ...
    document.getElementById("nIdObrisanog").value=konacniNiz[0]["TaskID"];
    document.getElementById("nIdObrisanog1").value=konacniNiz[0]["TaskID"];
      ...
      element.addEventListener("click", e=>{
        ...
        for (let index = 0; index < konacniBrojRedova; index++) {
          if(konacniNiz[index]['TaskID']==vrednost){
            document.getElementById("nIdObrisanog").value=konacniNiz[index]["TaskID"];
            document.getElementById("nIdObrisanog1").value=konacniNiz[index]["TaskID"];
